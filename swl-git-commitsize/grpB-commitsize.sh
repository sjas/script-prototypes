...
names=`echo "alexander"; echo "matthias"; echo "swt-028112"; echo "swt-035806"; `
...
for line in $(echo $names);do
    ...
    echo $author;
    commitsize=`git log -i --author="${author}" --stat | grep "java" | grep "|" | awk -F "|" '{print $2}' | awk -F " " '{print $1}' |  awk '{ SUM += $1} END { print SUM }'`;
    #echo $commitsize;
